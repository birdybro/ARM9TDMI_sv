...
| Instruction cycle summary tables | BASELINED | PARTIAL (profile-specific data-operation, CLZ, single/block/doubleword-transfer, PLD, swap, branch/exception refill, and MRS/MSR multicycle control) | VERIFIED four data-operation classes, ARM946E-S CLZ single cycle with ARM9TDMI rejection, LDR/STR, all 1-16 register LDM/STM classes, ARM946E-S LDRD/STRD normal and interlocked classes with ARM9TDMI rejection, ARM946E-S PLD single-cycle address/speculative indication with ARM9TDMI rejection, every published SWP interlock class and DLOCK window, branch/exception refill, and MRS/MSR latency/aggregate bus classes for both profiles plus ARM9E-S detailed cycle order; ARM9TDMI multi-cycle bus order is not fully published and remains explicitly unspecified; all other timing rows are not implemented |
