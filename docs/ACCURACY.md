...
| Exceptions and aborts | PARTIAL | PARTIAL (single/miscellaneous/doubleword/swap and LDM-PC precise Data Abort completion intent) | VERIFIED single/miscellaneous-load destination preservation, LDRD destination uncertainty reporting, base-restored writeback suppression for implemented transfers, SWP/SWPB read-abort store cancellation/destination suppression, and LDM PC/CPSR-restore suppression; general LDM destination uncertainty, exception entry, and timing not implemented |
