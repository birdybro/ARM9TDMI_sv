...
| Instruction cycle summary tables | BASELINED | PARTIAL (profile-specific data-operation, single/block-transfer, branch/exception refill, and MRS/MSR multicycle control) | VERIFIED four data-operation classes, LDR/STR, all 1-16 register LDM/STM classes, branch/exception refill, and MRS/MSR latency/aggregate bus classes for both profiles plus ARM9E-S detailed cycle order; ARM9TDMI multi-cycle bus order is not fully published and remains explicitly unspecified; all other timing rows are not implemented |
