...
| Instruction cycle summary tables | BASELINED | PARTIAL (profile-specific MRS/MSR multicycle control) | VERIFIED MRS/MSR latency and aggregate bus classes for both profiles plus ARM9E-S detailed cycle order; ARM9TDMI three-cycle MSR instruction-bus order is not published and remains explicitly unspecified; all other timing rows are not implemented |
