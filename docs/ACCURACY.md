...
| Instruction cycle summary tables | BASELINED | PARTIAL (profile-specific data-operation, CLZ, saturating arithmetic, multiply, single/block/doubleword-transfer, PLD, swap, branch/exception refill, and MRS/MSR multicycle control) | VERIFIED four data-operation classes, ARM946E-S CLZ and both saturating-arithmetic timing classes with ARM9TDMI rejection, every common multiply timing row plus ARM9E-S DSP multiply rows and their profile-specific early-termination/interlock rules, LDR/STR, all 1-16 register LDM/STM classes, ARM946E-S LDRD/STRD normal and interlocked classes with ARM9TDMI rejection, ARM946E-S PLD single-cycle address/speculative indication with ARM9TDMI rejection, every published SWP interlock class and DLOCK window, branch/exception refill, and MRS/MSR latency/aggregate bus classes for both profiles plus ARM9E-S detailed cycle order; ARM9TDMI multi-cycle bus order is not fully published and remains explicitly unspecified; all other timing rows are not implemented |
| Detailed hazards/interlocks | PARTIAL | NOT IMPLEMENTED | NOT IMPLEMENTED |
| ARM9TDMI multiplier termination | BASELINED | VERIFIED latency classifier and cycle sequencer | VERIFIED signed/unsigned class boundaries, all `m=1..4` cycle equations for every common multiply family, aggregate bus classes, and internal data cycles; chronological instruction-bus order is not published and remains explicit `UNSPECIFIED` |
| ARM946E-S multiplier timing | BASELINED | VERIFIED class/interlock classifier and cycle sequencer | VERIFIED fixed short/long/DSP classes, flag-setting paths, qualified dependency interlocks, and detailed per-cycle Tables 8-11 through 8-16 schedules |
