...
| ARM instruction set | PARTIAL | PARTIAL (data processing including PC completion, branch execute, common multiply execute, ARMv5TE DSP multiply execute, CLZ, saturating arithmetic, SWI, MRS/MSR execution, Addressing Modes 2/3/4, condition-gated LDM/STM descriptors, profile-separated LDRD/STRD paths, SWP/SWPB decode/preparation/completion, and common single/miscellaneous transfer paths) | VERIFIED ALU/shifter/immediate/decode integration, ordinary and exception-return PC completion, branches, multiply/DSP/CLZ/saturating/SWI side-effect intents, exhaustive MRS/MSR encoding classification, conditioned MRS reads, profile/privilege-filtered MSR writes and all MSR immediates, Addressing Modes 2/3/4, LDM/STM register/range/bank/writeback preparation, ARM946E-S LDRD/STRD paths with ARM9TDMI exclusion, exhaustive common SWP/SWPB decode, atomic descriptor, returned-data, and precise-abort intents, common single/miscellaneous request and completion intents, and load formatting; profile-specific PSR-transfer timing, block-transfer sequencing/completion, swap bus sequencing, external memory interfaces, cycle timing, and other classes not implemented |
...
| Instruction cycle summary tables | BASELINED | PARTIAL (profile-specific data-operation, single-transfer, branch/exception refill, and MRS/MSR multicycle control) | VERIFIED four data-operation classes, four LDR classes, STR, branch/exception refill, and MRS/MSR latency/aggregate bus classes for both profiles plus ARM9E-S detailed cycle order; ARM9TDMI multi-cycle bus order is not published and remains explicitly unspecified; all other timing rows are not implemented |
