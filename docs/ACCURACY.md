...
| ARM instruction set | PARTIAL | PARTIAL (non-PC data processing, branch execute, common multiply execute, ARMv5TE DSP multiply execute, CLZ, saturating-arithmetic execute, Addressing Modes 2/3, and single/miscellaneous transfer preparation/format/completion paths) | VERIFIED ALU/shifter/immediate/decode integration, branches, multiply/DSP/CLZ/saturating side-effect intents, Addressing Mode 2, common halfword/signed Addressing Mode 3, condition-gated LDR/STR/STRH/LDRH/LDRSB/LDRSH request/writeback intents, load formatting, and architectural completion intents; external memory interfaces, cycle timing, and other classes not implemented |
