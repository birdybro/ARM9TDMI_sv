...
| ARM instruction set | PARTIAL | PARTIAL (non-PC data processing, branch execute, common multiply execute, ARMv5TE DSP multiply execute, CLZ, saturating-arithmetic execute, and Addressing Mode 2 generation paths) | VERIFIED ALU/shifter/immediate/decode integration, B/BL/BX/BLX profile behavior, conditioned common/DSP multiply, CLZ, saturating writes/Q requests, and word/byte effective-address generation; memory execution and other classes not implemented |
