...
| Instruction cycle summary tables | BASELINED | PARTIAL (profile-specific data-operation and MRS/MSR multicycle control) | VERIFIED four data-operation classes and MRS/MSR latency/aggregate bus classes for both profiles plus ARM9E-S detailed cycle order; ARM9TDMI multi-cycle instruction-bus order is not published and remains explicitly unspecified; all other timing rows are not implemented |
