...
MRS_EXECUTE_RTL_SOURCES := rtl/arm9_arch_pkg.sv rtl/arm9_condition_eval.sv \
	rtl/arm9_psr_transfer_decoder.sv rtl/arm9_mrs_execute.sv
MSR_EXECUTE_RTL_SOURCES := rtl/arm9_profile_pkg.sv rtl/arm9_arch_pkg.sv \
	rtl/arm9_psr_pkg.sv rtl/arm9_condition_eval.sv \
	rtl/arm9_immediate_expander.sv rtl/arm9_psr_transfer_decoder.sv \
...
SATURATING_TIMING_RTL_SOURCES := rtl/arm9_profile_pkg.sv \
	rtl/arm9_timing_pkg.sv rtl/arm9_saturating_timing.sv
...
PLD_TIMING_RTL_SOURCES := rtl/arm9_profile_pkg.sv \
	rtl/arm9_timing_pkg.sv rtl/arm9_pld_timing.sv
...
DOUBLEWORD_TRANSFER_PREPARE_RTL_SOURCES := rtl/arm9_profile_pkg.sv \
	rtl/arm9_condition_eval.sv rtl/arm9_doubleword_transfer_decode.sv \
	rtl/arm9_doubleword_transfer_prepare.sv
DOUBLEWORD_TRANSFER_COMPLETE_RTL_SOURCES := \
	rtl/arm9_doubleword_transfer_complete.sv
SWAP_DECODER_RTL_SOURCES := rtl/arm9_swap_decoder.sv
SWAP_PREPARE_RTL_SOURCES := rtl/arm9_condition_eval.sv \
	rtl/arm9_swap_decoder.sv rtl/arm9_swap_prepare.sv
SWAP_COMPLETE_RTL_SOURCES := rtl/arm9_load_data_align.sv \
	rtl/arm9_swap_complete.sv
...
SATURATING_TIMING_TB := tb/unit/saturating_timing_tb.sv
...
PLD_TIMING_TB := tb/unit/pld_timing_tb.sv
...
DOUBLEWORD_TRANSFER_PREPARE_TB := tb/unit/doubleword_transfer_prepare_tb.sv
DOUBLEWORD_TRANSFER_COMPLETE_TB := tb/unit/doubleword_transfer_complete_tb.sv
SWAP_DECODER_TB := tb/unit/swap_decoder_tb.sv
SWAP_PREPARE_TB := tb/unit/swap_prepare_tb.sv
SWAP_COMPLETE_TB := tb/unit/swap_complete_tb.sv
...
.PHONY: all help toolchain spec lint compile test test-unit test-rtl-unit \
	...
	test-swap-decoder test-swap-prepare test-swap-complete test-formal synth \
	regression clean
...
help:
	...
	@echo "  test-saturating-timing test ARM946E-S QADD/QSUB cycles"
	...
	@echo "  test-pld-timing test ARM946E-S PLD bus cycles and DnSPEC"
	...
	@echo "  test-doubleword-transfer-prepare test conditioned LDRD/STRD descriptor"
	@echo "  test-doubleword-transfer-complete test LDRD/STRD commit and abort intent"
	@echo "  test-swap-decoder test exhaustive common SWP/SWPB decode"
	@echo "  test-swap-prepare test conditioned atomic swap descriptor"
	@echo "  test-swap-complete test returned data and precise-abort ordering"
...
lint: spec
	...
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
		--top-module exception_arbiter_tb \
		$(EXCEPTION_ARBITER_RTL_SOURCES) $(EXCEPTION_ARBITER_TB)
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
	...
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
		--top-module mrs_execute_tb \
		$(MRS_EXECUTE_RTL_SOURCES) $(MRS_EXECUTE_TB)
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
		--top-module msr_execute_tb \
		$(MSR_EXECUTE_RTL_SOURCES) $(MSR_EXECUTE_TB)
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
	...
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
		--top-module saturating_timing_tb \
		$(SATURATING_TIMING_RTL_SOURCES) $(SATURATING_TIMING_TB)
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
	...
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
		...
		$(DOUBLEWORD_TRANSFER_TIMING_TB)
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
		--top-module pld_timing_tb \
		$(PLD_TIMING_RTL_SOURCES) $(PLD_TIMING_TB)
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
	...
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
		--top-module doubleword_transfer_prepare_tb \
		$(DOUBLEWORD_TRANSFER_PREPARE_RTL_SOURCES) \
		$(DOUBLEWORD_TRANSFER_PREPARE_TB)
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
		--top-module doubleword_transfer_complete_tb \
		$(DOUBLEWORD_TRANSFER_COMPLETE_RTL_SOURCES) \
		$(DOUBLEWORD_TRANSFER_COMPLETE_TB)
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
		--top-module swap_decoder_tb \
		$(SWAP_DECODER_RTL_SOURCES) $(SWAP_DECODER_TB)
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
		--top-module swap_prepare_tb \
		$(SWAP_PREPARE_RTL_SOURCES) $(SWAP_PREPARE_TB)
	$(VERILATOR) --lint-only --Wall --assert --timing --timescale 1ns/1ps \
		--top-module swap_complete_tb \
		$(SWAP_COMPLETE_RTL_SOURCES) $(SWAP_COMPLETE_TB)
...
$(BUILD_DIR)/address_mode4/Vaddress_mode4_tb: \
	$(ADDRESS_MODE4_RTL_SOURCES) $(ADDRESS_MODE4_TB)
	@mkdir -p $(BUILD_DIR)/address_mode4
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/address_mode4 \
		--top-module address_mode4_tb \
		$(ADDRESS_MODE4_RTL_SOURCES) $(ADDRESS_MODE4_TB)
...
$(BUILD_DIR)/block_pc_complete/Vblock_pc_complete_tb: \
	$(BLOCK_PC_COMPLETE_RTL_SOURCES) $(BLOCK_PC_COMPLETE_TB)
	@mkdir -p $(BUILD_DIR)/block_pc_complete
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/block_pc_complete \
		--top-module block_pc_complete_tb \
		$(BLOCK_PC_COMPLETE_RTL_SOURCES) $(BLOCK_PC_COMPLETE_TB)
...
$(BUILD_DIR)/exception_arbiter/Vexception_arbiter_tb: \
	$(EXCEPTION_ARBITER_RTL_SOURCES) $(EXCEPTION_ARBITER_TB)
	@mkdir -p $(BUILD_DIR)/exception_arbiter
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/exception_arbiter \
		--top-module exception_arbiter_tb \
		$(EXCEPTION_ARBITER_RTL_SOURCES) $(EXCEPTION_ARBITER_TB)

$(BUILD_DIR)/swi_execute/Vswi_execute_tb: \
	$(SWI_EXECUTE_RTL_SOURCES) $(SWI_EXECUTE_TB)
	@mkdir -p $(BUILD_DIR)/swi_execute
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/swi_execute \
		--top-module swi_execute_tb \
		$(SWI_EXECUTE_RTL_SOURCES) $(SWI_EXECUTE_TB)
...
$(BUILD_DIR)/mrs_execute/Vmrs_execute_tb: \
	$(MRS_EXECUTE_RTL_SOURCES) $(MRS_EXECUTE_TB)
	@mkdir -p $(BUILD_DIR)/mrs_execute
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/mrs_execute \
		--top-module mrs_execute_tb \
		$(MRS_EXECUTE_RTL_SOURCES) $(MRS_EXECUTE_TB)

$(BUILD_DIR)/msr_execute/Vmsr_execute_tb: \
	$(MSR_EXECUTE_RTL_SOURCES) $(MSR_EXECUTE_TB)
	@mkdir -p $(BUILD_DIR)/msr_execute
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/msr_execute \
		--top-module msr_execute_tb \
		$(MSR_EXECUTE_RTL_SOURCES) $(MSR_EXECUTE_TB)

$(BUILD_DIR)/psr_transfer_timing/Vpsr_transfer_timing_tb: \
	$(PSR_TRANSFER_TIMING_RTL_SOURCES) $(PSR_TRANSFER_TIMING_TB)
	@mkdir -p $(BUILD_DIR)/psr_transfer_timing
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/psr_transfer_timing \
		--top-module psr_transfer_timing_tb \
		$(PSR_TRANSFER_TIMING_RTL_SOURCES) $(PSR_TRANSFER_TIMING_TB)
...
$(BUILD_DIR)/saturating_timing/Vsaturating_timing_tb: \
	$(SATURATING_TIMING_RTL_SOURCES) $(SATURATING_TIMING_TB)
	@mkdir -p $(BUILD_DIR)/saturating_timing
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/saturating_timing \
		--top-module saturating_timing_tb \
		$(SATURATING_TIMING_RTL_SOURCES) $(SATURATING_TIMING_TB)
...
$(BUILD_DIR)/pipeline_refill_timing/Vpipeline_refill_timing_tb: \
	...
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/pipeline_refill_timing \
		--top-module pipeline_refill_timing_tb \
		$(PIPELINE_REFILL_TIMING_RTL_SOURCES) $(PIPELINE_REFILL_TIMING_TB)
...
$(BUILD_DIR)/block_transfer_timing/Vblock_transfer_timing_tb: \
	$(BLOCK_TRANSFER_TIMING_RTL_SOURCES) $(BLOCK_TRANSFER_TIMING_TB)
	@mkdir -p $(BUILD_DIR)/block_transfer_timing
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/block_transfer_timing \
		--top-module block_transfer_timing_tb \
		$(BLOCK_TRANSFER_TIMING_RTL_SOURCES) $(BLOCK_TRANSFER_TIMING_TB)
...
$(BUILD_DIR)/pld_timing/Vpld_timing_tb: \
	$(PLD_TIMING_RTL_SOURCES) $(PLD_TIMING_TB)
	@mkdir -p $(BUILD_DIR)/pld_timing
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/pld_timing \
		--top-module pld_timing_tb \
		$(PLD_TIMING_RTL_SOURCES) $(PLD_TIMING_TB)
...
$(BUILD_DIR)/misc_transfer_complete/Vmisc_transfer_complete_tb: \
	$(MISC_TRANSFER_COMPLETE_RTL_SOURCES) $(MISC_TRANSFER_COMPLETE_TB)
	@mkdir -p $(BUILD_DIR)/misc_transfer_complete
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/misc_transfer_complete \
		--top-module misc_transfer_complete_tb \
		$(MISC_TRANSFER_COMPLETE_RTL_SOURCES) $(MISC_TRANSFER_COMPLETE_TB)
...
$(BUILD_DIR)/doubleword_transfer_prepare/Vdoubleword_transfer_prepare_tb: \
	$(DOUBLEWORD_TRANSFER_PREPARE_RTL_SOURCES) $(DOUBLEWORD_TRANSFER_PREPARE_TB)
	@mkdir -p $(BUILD_DIR)/doubleword_transfer_prepare
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/doubleword_transfer_prepare \
		--top-module doubleword_transfer_prepare_tb \
		$(DOUBLEWORD_TRANSFER_PREPARE_RTL_SOURCES) \
		$(DOUBLEWORD_TRANSFER_PREPARE_TB)

$(BUILD_DIR)/doubleword_transfer_complete/Vdoubleword_transfer_complete_tb: \
	$(DOUBLEWORD_TRANSFER_COMPLETE_RTL_SOURCES) $(DOUBLEWORD_TRANSFER_COMPLETE_TB)
	@mkdir -p $(BUILD_DIR)/doubleword_transfer_complete
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/doubleword_transfer_complete \
		--top-module doubleword_transfer_complete_tb \
		$(DOUBLEWORD_TRANSFER_COMPLETE_RTL_SOURCES) \
		$(DOUBLEWORD_TRANSFER_COMPLETE_TB)

$(BUILD_DIR)/swap_decoder/Vswap_decoder_tb: \
	$(SWAP_DECODER_RTL_SOURCES) $(SWAP_DECODER_TB)
	@mkdir -p $(BUILD_DIR)/swap_decoder
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/swap_decoder \
		--top-module swap_decoder_tb \
		$(SWAP_DECODER_RTL_SOURCES) $(SWAP_DECODER_TB)

$(BUILD_DIR)/swap_prepare/Vswap_prepare_tb: \
	$(SWAP_PREPARE_RTL_SOURCES) $(SWAP_PREPARE_TB)
	@mkdir -p $(BUILD_DIR)/swap_prepare
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/swap_prepare \
		--top-module swap_prepare_tb \
		$(SWAP_PREPARE_RTL_SOURCES) $(SWAP_PREPARE_TB)

$(BUILD_DIR)/swap_complete/Vswap_complete_tb: \
	$(SWAP_COMPLETE_RTL_SOURCES) $(SWAP_COMPLETE_TB)
	@mkdir -p $(BUILD_DIR)/swap_complete
	$(VERILATOR) $(VERILATOR_COMMON) --timing \
		--Mdir $(BUILD_DIR)/swap_complete \
		--top-module swap_complete_tb \
		$(SWAP_COMPLETE_RTL_SOURCES) $(SWAP_COMPLETE_TB)
...
test-exception-arbiter: $(BUILD_DIR)/exception_arbiter/Vexception_arbiter_tb
	$(BUILD_DIR)/exception_arbiter/Vexception_arbiter_tb
...
test-mrs-execute: $(BUILD_DIR)/mrs_execute/Vmrs_execute_tb
	$(BUILD_DIR)/mrs_execute/Vmrs_execute_tb

test-msr-execute: $(BUILD_DIR)/msr_execute/Vmsr_execute_tb
	$(BUILD_DIR)/msr_execute/Vmsr_execute_tb
...
test-pld-timing: $(BUILD_DIR)/pld_timing/Vpld_timing_tb
	$(BUILD_DIR)/pld_timing/Vpld_timing_tb
...
test-doubleword-transfer-prepare: $(BUILD_DIR)/doubleword_transfer_prepare/Vdoubleword_transfer_prepare_tb
	$(BUILD_DIR)/doubleword_transfer_prepare/Vdoubleword_transfer_prepare_tb

test-doubleword-transfer-complete: $(BUILD_DIR)/doubleword_transfer_complete/Vdoubleword_transfer_complete_tb
	$(BUILD_DIR)/doubleword_transfer_complete/Vdoubleword_transfer_complete_tb

test-swap-decoder: $(BUILD_DIR)/swap_decoder/Vswap_decoder_tb
	$(BUILD_DIR)/swap_decoder/Vswap_decoder_tb

test-swap-prepare: $(BUILD_DIR)/swap_prepare/Vswap_prepare_tb
	$(BUILD_DIR)/swap_prepare/Vswap_prepare_tb

test-swap-complete: $(BUILD_DIR)/swap_complete/Vswap_complete_tb
	$(BUILD_DIR)/swap_complete/Vswap_complete_tb
...
test-rtl-unit: test-condition test-register-file test-status-registers test-shifter \
	...
	test-misc-transfer-complete test-doubleword-transfer-decode \
	test-doubleword-transfer-prepare test-doubleword-transfer-complete \
	test-swap-decoder test-swap-prepare test-swap-complete
